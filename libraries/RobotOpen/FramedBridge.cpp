...
void FramedBridgeClass::parseFrame() {
    ...
    unsigned int crc16_recv = (_incomingPacket[_incomingPacketIndex - 2] << 8) | _incomingPacket[_incomingPacketIndex - 1];
    
    // calculate our own checksum and compare
    if (calc_crc16(_incomingPacket, _incomingPacketIndex - 2) == crc16_recv) {
        // valid packet!
        if (onFrame)
            onFrame(_incomingPacket, _incomingPacketIndex - 2);
    }
}
...
void FramedBridgeClass::send() {
    ...
    _outgoingPacket[_outgoingPacketIndex++] = (crc16_xmit >> 8) & 0xFF;
    _outgoingPacket[_outgoingPacketIndex++] = crc16_xmit & 0xFF;
...
unsigned int FramedBridgeClass::calc_crc16(unsigned char *buf, unsigned short len) {
